...
$bc_big_small_number_rand=array_rand(range(1, 99),3);
shuffle($bc_big_small_number_rand);
$bc_big_small_sort_var=$bc_big_small_number_rand;
//$bc_big_small_value_display = preg_split('/\s+/', $bc_big_small_number_rand);
...
//Checking for Increasing or Decreasing order
	...
	echo "<lablel>"._e('<b>*</b> Please enter the <b>Biggest Number</b><br>','bc_big_small_simple_captcha')."</label><br>";
	}
	else {
	$bc_big_small_sort_var_min=min($bc_big_small_sort_var);
	$bc_big_small_value_passing=$bc_big_small_sort_var_min;
	echo "<lablel>"._e('<b>*</b> Please enter the <b>Smallest Number</b><br>','bc_big_small_simple_captcha')."</label><br>";
	}

echo "<lablel>";
echo $bc_big_small_value_display;
echo "</label>    "; 
echo "<input id=\"bc_big_small_captcha_input\" type=\"text\" autocomplete=\"off\" name=\"bc_big_small_captcha_value\" value=\"\" maxlength=\"5\" size=\"15\" aria-required=\"true\" style=\"margin-bottom:0;display:inline;font-size: 12px;width: 90px;\" />"; ?>
<input type="hidden" value="<?php echo $bc_big_small_value_passing; ?>" name="bc_big_small_captcha_rand_value" />
<?php
}	
...
if ($_REQUEST['bc_big_small_captcha_value'] =="" && isset($_REQUEST['bc_big_small_captcha_value'] ))
			wp_die( __('Please Enter Captcha.', 'bc_big_small_simple_captcha' ) );

// Checking for Captcha matching			
if ($_REQUEST['bc_big_small_captcha_value']==$_REQUEST['bc_big_small_captcha_rand_value']) {
		return($comment);
}
else {
		wp_die( __('Error: Incorrect CAPTCHA, Please click back and reenter', 'bc_big_small_simple_captcha'));
}
}
