...
echo "<label>"._e('<b>*</b> Please enter the <b>Output</b><br>','bc_add_sub_simple_captcha')."</label><br>";
...
echo "</label>    "; 
echo "<input id=\"bc_add_sub_captcha_input\" type=\"text\" autocomplete=\"off\" name=\"bc_add_sub_captcha_value\" value=\"\" maxlength=\"3\" size=\"15\" aria-required=\"true\" style=\"margin-bottom:0;display:inline;font-size: 12px;width: 90px;\" />"; ?>
<input type="hidden" value="<?php echo bc_encrypt($bc_add_sub_passing); ?>" name="bc_add_sub_captcha_rand_value" />
<?php
}	
...
if ($_REQUEST['bc_add_sub_captcha_value'] =="" && isset($_REQUEST['bc_add_sub_captcha_value'] ))
			wp_die( __('Please Enter Big or Small Value Captcha.', 'bc_add_sub_simple_captcha' ) );

// Checking for Captcha matching			
if ($_REQUEST['bc_add_sub_captcha_value']==bc_decrypt($_REQUEST['bc_add_sub_captcha_rand_value'])) {
		return($comment);
}
else {
		//die(print_r($_REQUEST['bc_add_sub_captcha_value']));
		wp_die( __('Error: Incorrect CAPTCHA, Please click back and reenter', 'bc_add_sub_simple_captcha'));
}
}
