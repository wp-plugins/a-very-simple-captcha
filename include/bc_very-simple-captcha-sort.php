...
//Checking for Increasing or Decreasing order
	...
	$bc_sort_value_passing=implode("",$bc_sort_sort_var);
	echo "<label>"._e('<b>*</b> Please arrange the below number in <b>decreasing order</b><br>','bc_sort_simple_captcha')."</label><br>";
	}
...
<input type="hidden" value="<?php echo bc_encrypt($bc_sort_value_passing); ?>" name="bc_sort_captcha_rand_value" />
<?php
}	
...
if ($_REQUEST['bc_sort_captcha_value'] =="" && isset($_REQUEST['bc_sort_captcha_value'] ))
			wp_die( __('Please Enter Sort Captcha.', 'bc_sort_simple_captcha' ) );

// Checking for Captcha matching			
if ($_REQUEST['bc_sort_captcha_value']==bc_decrypt($_REQUEST['bc_sort_captcha_rand_value'])){
		return($comment);
}
else {
		//die(print_r($_REQUEST['bc_sort_captcha_value']));
