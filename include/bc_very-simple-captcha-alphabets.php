...
<input type="hidden" value="<?php echo $bc_alphabets_value_passing_1; ?>" name="bc_alphabets_captcha_rand_value_1" />
<input type="hidden" value="<?php echo $bc_alphabets_value_passing_2; ?>" name="bc_alphabets_captcha_rand_value_2" />
<?php
}	
...
if ($_REQUEST['bc_alphabets_captcha_value'] =="" && isset($_REQUEST['bc_alphabets_captcha_value'] ))
			wp_die( __('Please Enter Captcha.', 'bc_alphabets_simple_captcha' ) );

// Checking for Captcha matching			
if (strtolower($_REQUEST['bc_alphabets_captcha_value'])==$_REQUEST['bc_alphabets_captcha_rand_value_1'] ||strtoupper($_REQUEST['bc_alphabets_captcha_value'])==$_REQUEST['bc_alphabets_captcha_rand_value_2']  ) {
		return($comment);
}
else {
